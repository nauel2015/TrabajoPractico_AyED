...
void rankingProductosMasVendidos()
{
    ...
    for (int i = 0; i < lenProductos - 1; i++)
    {
        for (int j = i + 1; j < lenProductos; j++)
        {
            if (productos[j].cantidad > productos[i].cantidad)
            {
             ProductoContador aux = productos[i];
    ...
    //Mostrar Ranking
    cout << "Ranking de productos más vendidos: " << endl;
    for (int i = 0; i < lenProductos; i++)
    {
        cout << "Producto " << productos[i].codigoProducto 
        << " - Vendido " << productos[i].cantidad << " veces " << endl;
    }
